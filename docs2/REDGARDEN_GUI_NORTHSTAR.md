...
**Server stays in REDGARDEN; the client is a real fork, not a dependency (2026-08-02).**
`apps/arena_server`/`apps/matchmaker` above are reused as external processes, spawned from
REDGARDEN's own repo -- that part is correctly "unchanged, not forked." The GUI client is
different: it now lives at `apps2/battlegrounds_gui/` in **this** repo, a real standalone copy
of REDGARDEN's `apps/arena` (forked at commit `61baafb`), not a live checkout of the REDGARDEN
repo pulled in at CI build time (that was tried first and corrected -- founder: "REDGARDEN isnt
literally the GUI its supposed to be a starting place for the GUI like a clean fork"). It's
self-contained (its own `packages/simulation` + `packages/common`, not sharing GFD's existing
top-level `packages/`/`packages2/`, which have unrelated real content of their own) and can
diverge from REDGARDEN going forward, the same way `apps/lobby`'s own SHANKPIT fork already can.
GFD's own CI (`GoblinFoxDragon Factory`) builds it directly, no cross-repo checkout step.
...
| 5 | End-to-end validated | A real persistent-world character queues, picks Warrior, plays a real match casting real weapon skills through REDGARDEN's UI, chains a real skillchain, and returns to the persistent world with a real credited reward | **PARTIAL** 2026-08-02 — the identity/ticket chain is live-verified end-to-end and fast (ms). The manual "run REDGARDEN's client by hand with a printed command" step this milestone originally accepted is gone: a real GUI login screen (originally REDGARDEN `9c98342` + Winsock fix `e6fb748`, now forked into `apps2/battlegrounds_gui/` per §4.1's own note) now does that step itself. Also found and fixed a real, severe bug while validating this live: IDUNA's own `REDGARDEN_TICKET_SECRET` didn't match the value all three live REDGARDEN systemd units actually verify against (`test-secret-for-vs0-vs1-validation`) -- every IDUNA-minted ticket, from any endpoint, had never been able to verify against the live game server; bots were silently masking this by falling back to self-minted tickets. Fixed by aligning IDUNA's secret to the live deployment's own value. Verified live end-to-end with a real working test account (`test@test.com`/`testtest`): login → self-ticket mint → connect → **CLIENT reaches "20/20 connected"** in a real live match. Still open: the full interactive match-play half (draft-pick → cast → chain → credit) needs either GUI-input automation or a skillchain-aware bot, neither built here (see §9) |
