...
int main(int argc, char* argv[]) {
    ...
    while(running) {
        ...
        while(SDL_PollEvent(&e)) {
            ...
            if (app_state == STATE_LOBBY) {
                if(e.type == SDL_KEYDOWN) {
                    if (e.key.keysym.sym == SDLK_1) { app_state = STATE_GAME_LOCAL; local_init_match(8, MODE_DEATHMATCH); }
        ...
        if (app_state == STATE_LOBBY) {
             ...
             glBegin(GL_LINES); glVertex2f(200, 300); glVertex2f(200, 400); glEnd(); // Minimal UI
