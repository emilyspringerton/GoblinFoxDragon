...
func initWorld() *world {
	...
	// (spawn.NewRegistry's own real fail-open default), matching the exact pre-existing
	...
	spawnInto(0, mob.MeadowWormSpawns())
	// S189-07: seeds the Meadow with goblin/fox NPCs generated by apps2/crystal's
	// own boids/ecosystem simulation, if a seed file has been produced (see
	// apps2/crystal/main.go's `-seed`/`-seed-out` flags). Optional/additive --
	// MeadowCrystalSpawns returns (nil, nil) if the env var is unset or the file
	// doesn't exist, so world init never fails or changes behavior without one.
	if crystalMobs, err := mob.MeadowCrystalSpawns(os.Getenv("CRYSTAL_SEED_PATH")); err != nil {
		log.Printf("crystal seed load failed (non-fatal, Meadow just won't have crystal NPCs): %v", err)
	} else {
		spawnInto(0, crystalMobs)
	}
	// Zone 1 (Hills) and Zone 2 (Caves): real, found-live bug fixed in the same pass that added
	// spawnReg above -- HillsSpawns() and CavesSpawns() have existed since S189's own hero-lineup
	// work but were never actually called from world init, so these two zones have been silently
	// empty (registered, reachable, but mobless) in the live server the entire time. First real
	// population for both.
	spawnInto(1, mob.HillsSpawns())
	spawnInto(2, mob.CavesSpawns())
	spawnInto(3, mob.SwampvilleSpawns())
