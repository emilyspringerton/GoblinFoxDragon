...
- Fixed a real, live bug (founder screenshot: 'help' rendering as a garbled, staggered mess in a real Windows Git Bash/MINGW64 terminal over SSH): every multi-line send() ever sent a message with only ONE trailing CRLF, leaving every internal line break as a bare LF -- fine for telnet clients (which conventionally translate LF to CRLF themselves) but broken for a real Windows PTY-based SSH session, which does not. New normalizeLineEndings() (a real two-pass normalize, not a naive single ReplaceAll -- ~120 existing call sites already prepend a correct leading CRLF for spacing and must not get doubled), wired into player.send and both structurally identical local send closures (handleConn, runSSHClaimFlow). 3 new tests. Isolated from a concurrent in-flight main.go change, go build/vet/test ./... clean. Live-verified on a throwaway instance: raw socket capture confirms zero bare LF bytes remain in a real help response. Commit e652950 (source pushed; deploy deliberately held -- two live demo SSH sessions currently running for the founder's own stream). (sess-20260905-0720-ec33e7c5)
- Fixed a real, found-live gap: Home Point never persisted across a real SSH reconnect (founder: 'home point does not persist after logouts'). applyFetchedCharacter -- the function every real SSH reconnect goes through -- never restored p.homePoint from the character record, even though sethome correctly persisted it to IDUNA already. getOrCreateHeadlessPlayer's own connect path had this fix since 2026-08-04 but it was never applied to the real player-facing path. Extracted into a shared restoreHomePointFromCharacter used by both paths now. 2 new tests. Found but not fixed in the same pass: the telnet mudCharCache returning-guest branch has the identical gap, entangled with an unrelated in-flight change, flagged for next pickup. Commit bd5a7a6 (source, not yet deployed -- holding the restart so it doesn't drop two live demo SSH sessions currently running for the founder). (sess-20260905-0720-ec33e7c5)
- Rapid live-fix chain during real-time SSH testing/streaming (2026-09-12): (1) TCP_NODELAY on both listeners -- fixes 'have to hit enter twice' (Nagle+delayed-ACK interacting with the new per-keystroke echo). (2) CRITICAL: readTerminalLine's r.Peek(1) after a bare CR blocked forever when nothing followed it -- a real production deadlock on every line read, found live seconds after the echo fix shipped, fixed by only peeking when r.Buffered() > 0. (3) who listing now tags [Guest] players. All isolated from a concurrent in-flight main.go change, all live-verified against real production (including a real ssh-keyscan-confirmed round trip proving the deadlock fix: a fresh key claiming a taken name now correctly gets 'That name is already taken' instead of hanging). Commits 79b5283/bd93d40/fd3c601 (source) + 28a93be (binary redeploy). (sess-20260905-0720-ec33e7c5)
- Guest-mode messaging now gives the real SSH connect command instead of stale 'coming soon' wording: new sshUpgradeHint constant ('ssh -p 2222 okemily.com...') used consistently in the connect banner, economy/chat block messages, and requireSSHIdentity's own key-management refusal. New regression test guards against 'coming soon' ever creeping back in. Live-verified. Commits cb63dc4 (source) + b732089 (binary redeploy). (sess-20260905-0720-ec33e7c5)
- Fixed a real, live SSH bug (founder report: 'it wont let me type' at the claim flow's Name prompt over a real Windows OpenSSH client): every raw-input read (guest name prompt, main command loop, SSH claim flow) used bufio.Reader.ReadString('\n'), which never echoes and never terminates on a bare CR -- exactly what a real PTY client sends on Enter with no local echo of its own. New apps2/mud/terminal_io.go: readTerminalLine does real line editing (backspace actually removes from the buffer) + CR/LF normalization + optional per-keystroke echo (on only for a real PTY-negotiated SSH session, off for telnet which already echoes locally). 11 new tests, go test ./... clean. Live-verified with a real ssh binary, a real PTY, and a character-by-character write ending in a bare CR (not the whole-string LF write this session's earlier verification used, which sidestepped both real bugs) -- server log confirms the claim completed. Also fixed a self-inflicted side effect from that same testing: a stray 'rm ~/.ssh/known_hosts' broke this repo's own git push over SSH, restored via ssh-keyscan against GitHub's published host keys. Commits 9752bdb (source) + 4eb47a7 (binary redeploy). (sess-20260905-0720-ec33e7c5)
- Real cast time for WHM/RDM/BLM spells (JOB_SPELL_SYSTEM_NORTHSTAR.md §1 Phase 1 remainder): new apps2/mud/ability.go -- spellCastTimes gives every already-shipped WHM/RDM/BLM spell a real, non-zero cast time for the first time (was instant, gated only by the 1s universal lockout). beginCast defers the exact existing effect logic via time.AfterFunc after a fast MP/target preflight; checkNotCasting is a real shared gate both cmdCast and cmdJA now call. 11 new tests, go test ./... clean. Live-verified in production: 'cast cure' shows 'You begin casting Cure... (2s)', an immediate ja attempt is refused ('already casting'), and the effect lands automatically ~2s later. Isolated from a concurrent in-flight main.go change via the established hunk technique. Commit 8b4f561. (sess-20260905-0720-ec33e7c5)
- SSH login guest-fallback bug fixed: ~/.config/gfd-mud/env (IDUNA_AGENT_NAME/SECRET) had vanished from disk (home-dir reset, file lives outside git+outside the gfd backup target), so every idunaclient call was silently 401ing and every real SSH connection fell back to guest with the 'character does not save' banner. Re-created from IDUNA/var/agent-secrets.env, restarted gfd-mud.service, live-verified: fresh key claims a character, reconnect with the same key now resumes it (no guest fallback). (sess-20260905-0720-ec33e7c5)
- feat(jobs): real effects for Berserk/Boost/Sneak Attack (JOB_SPELL_SYSTEM_NORTHSTAR.md Phase 1, §0.5 fixes) -- Berserk now applies real status.Haste (+30%, 3m); Boost/Sneak Attack get real one-shot pendingAttackBonus/pendingGuaranteedCrit consumed by the caster's own next LANDED auto-attack (not consumed by a miss). Elemental Seal/Chainspell/Trick Attack remain flavor-text-only, each named with its own real, larger reason. 8 new tests; live-verified (Boost produced a real 31-damage hit vs. the surrounding 20-26 range). (sess-20260905-0720-ec33e7c5)
- feat(combat): real STR/DEX/VIT/AGI damage formula w/ marble-bag+pity (JOB_SPELL_SYSTEM_NORTHSTAR.md Phase 4.5) -- new apps2/mud/combat_formula.go replaces the flat playerDamage=30/per-mob-MeleeDamage deterministic hits with STR-scaled random player damage, DEX-based crit (1.5x), a shared marble-bag+pity accuracy/evasion roll (pity always favors the player, attacking or defending), and VIT-based reduction. New server/mob.Registry.ReadyToSwing split out of TickPlayer (unchanged behavior, verified by regression test) so apps2/mud can compute the real roll. Mobs get a flat accuracy/crit baseline (real v0 answer to the open stat-block question). PvP duels untouched (separate mechanic, named scope boundary). 16 new tests total; live-verified twice (throwaway instance + real production): real variance (24-28 dmg vs. the old flat 30), real crits and evades on both sides, full kill/XP/loot/KO flow unaffected. (sess-20260905-0720-ec33e7c5)
- feat(combat): universal 1s spell/ability lockout (JOB_SPELL_SYSTEM_NORTHSTAR.md §1) -- new p.lastActionAt + checkUniversalLockout gating both cmdCast and cmdJA (covers all six of cmdCast's own delegate spell functions too). A player can no longer chain-spam abilities with zero real gap. 7 new tests; live-verified (back-to-back ja provoke blocked with a real countdown, clears after 1.1s). (sess-20260905-0720-ec33e7c5)
- feat(rng): shared weighted marble-bag + Fibonacci pity utility (server/rng) -- Go port of ECOWAR's arena_fibonacci/arena_marble_bag_pick for combat's hit/miss and crit rolls (per the founder's own "one mechanism, two call sites... worth building once as a shared utility" note). Found+documented a real pre-existing off-by-one in ECOWAR's own header comment (fib(10) claimed 55, real code computes 89) via test. 7 new tests. Design for wiring into combat in JOB_SPELL_SYSTEM_NORTHSTAR.md §5; not yet wired into real combat code. (sess-20260905-0720-ec33e7c5)
- feat(jobs): disable advanced jobs, plan job/spell system overhaul (Phase 0) -- new docs2/JOB_SPELL_SYSTEM_NORTHSTAR.md: real audit found most complaint-cited spells/abilities already work (BLM elementals, WAR provoke, MNK boost/chakra, THF sneak attack, WHM/RDM cure/haste/dia/refresh), but zero cast-time/cooldown/lockout system exists anywhere, and about half the job-ability roster (berserk/boost/elemental seal/chainspell/sneak attack/trick attack) sends a flavor-text message with no real effect applied. Also folded in a second founder finding: auto-attack damage is a flat hardcoded 30 on both combat directions, zero stat/variance -- real proposed formula given (STR-scaled+random damage, DEX crit, shared accuracy/evasion roll, VIT reduction). Phase 0 shipped: setjob/setjob-sub refuse every job outside WAR/MNK/WHM/BLM/RDM/THF (ASN named explicitly), existing characters on a disabled job left alone. Phases 1-5 scoped with real open questions for the founder. (sess-20260905-0720-ec33e7c5)
- feat(security): real SSH identity binding + claim flow (SSH Transport & Identity spec, Stage 5 / §3) -- new IDUNA character_ssh_keys table + /api/v1/ssh-keys endpoints; GFD's handleConn now takes (isGuest, preset), resolving a known fingerprint to its bound character (no prompt) or running a rate-limited, validated claim flow for an unknown one. New in-session `keys`/`key-add`/`key-revoke` commands. Live-verified end-to-end against real production IDUNA: claim, resume, name-collision refusal+reprompt, key-add/revoke, revoked-key-can't-resume, and the real per-IP rate limit all confirmed. 11 new tests across IDUNA/idunaclient/apps2-mud. (sess-20260905-0720-ec33e7c5)
- feat(security): real SSH listener, public-key/TOFU only (SSH Transport & Identity spec, Stage 4 / §2) -- new apps2/mud/ssh_listener.go, wired in alongside telnet on :2222. Password/keyboard-interactive auth refused; any offered public key accepted (TOFU, identity binding is Stage 5); host key persisted at var/ssh_host_ed25519_key; pty-req/window-change acknowledged, shell degrades cleanly with no PTY; idle timeout (15m)/max session (6h)/max 5 concurrent sessions per IP/MaxAuthTries=3. handleConn() reused completely unmodified for SSH sessions via a thin net.Conn adapter -- same guest gate, same IDUNA fetch-or-create. 7 new tests; live-verified against a throwaway instance and then the real production service (founder-authorized deploy). (sess-20260905-0720-ec33e7c5)
- chore(mud): rebuild + deploy live binary -- SSH listener now live (Stage 4). (sess-20260905-0720-ec33e7c5)
- chore(mud): rebuild + deploy live binary -- catches up gfd-mud.service to Stage 1/2 fixes (mob Revive, item-use fixes, per-job leveling, guest gate) that had been committed+tested but never actually deployed since 2026-09-05. Found while restarting for Stage 3 below. Live-verified via real socket connect. (sess-20260905-0720-ec33e7c5)
- feat(security): real systemd sandboxing for gfd-mud.service (SSH Transport & Identity spec, Stage 3 / §5, partial) -- ProtectSystem=strict+ReadWritePaths=var confines all writes to the one real data path; plus PrivateTmp/RestrictNamespaces/SUIDSGID/Realtime/AddressFamilies/LockPersonality/RemoveIPC/ProtectHostname/KernelTunables/ControlGroups/a seccomp allowlist/NoNewPrivileges/UMask. Found live (tested each directive individually): CapabilityBoundingSet=/AmbientCapabilities=/ProtectKernelModules=/ProtectKernelLogs=/ProtectClock= all fail under `systemctl --user` (no CAP_SETPCAP) -- queued sudo-queue/78 for the root-requiring dedicated-user+full-capability-drop follow-up. Backed up var/ first (new emily.cli `gfd` backup target + local tarball, restore round-trip verified). Exposure score 9.8 UNSAFE -> 5.8 MEDIUM, live-verified after restart. (sess-20260905-0720-ec33e7c5)
- feat(security): disable plaintext GUI login (SSH Transport & Identity spec, Stage 2 / §7 "blocking item") -- apps2/battlegrounds_gui/src/main.c's email+password login screen sends credentials over a raw BSD-socket HTTP client with zero TLS on either build target (Linux dev, mingw-w64 Windows CI). Real TLS in that client would need vendoring+cross-compiling OpenSSL/mbedTLS plus real cert verification, not safely rushed. Per the founder's own live choice, GUI login is disabled instead (kGuiLoginDisabledPendingTLS, gated at UI, input-handling, AND the actual network-call site) until real TLS lands -- no email/password field is even drawn, SIGN UP (browser-based, real TLS via the OS browser) is unaffected. Live-verified: native Linux build of the exact CI source list, screenshotted under Xvfb showing the disabled notice with no credential fields. (sess-20260905-0720-ec33e7c5)
- feat(security): telnet guest-tier economy+chat gate (SSH Transport & Identity spec, Stage 1) -- guestGate blocks bank/bazaar/ah/say/tell/yell/guild/party-chat for every anonymous telnet connection at the real server dispatch layer, explaining the SSH upgrade path per refusal. Connect banner states guest scope + real non-persistence behavior up front. Full spec + phased plan in docs2/SSH_TRANSPORT_IDENTITY_*.md. 11 new tests, live-verified via direct socket testing. (sess-20260905-0720-ec33e7c5)
- feat(job): per-job leveling -- each job levels independently (GFD-124433). p.charXP is now a live alias into a new p.jobXP map, switched via switchActiveJob/loadJobXP (both unit-tested). Persists via IDUNA's new character_job_levels table. Two twin bugs fixed: stale p.charJob.MainLvl on job switch, missing applyJobStats on telnet-reconnect. 11 new tests. (sess-20260905-0720-ec33e7c5)
- fix(mud): Hi-Potion/Potion/Ether/Echo Drop/Antidote now have real effects via 'use' -- cmdUseItem previously reported 'nothing happens yet' for every consumable; Echo Drop was already implemented but only reachable via removedebuffs/erase, not use. Also fixed use/eat's dispatch never lowercasing item-id args. GitHub-reported: 'the mud items arent implemented like echo drops and hi potion.' (sess-20260905-0720-ec33e7c5)
