...
void apply_friction(PlayerState *p) {
    ...
    float drop = 0;
    if (p->in_vehicle) drop = speed * BUGGY_FRICTION;
    else if (p->on_ground) {
        if (p->crouching) {
...
void accelerate(PlayerState *p, float wish_x, float wish_z, float wish_speed, float accel) {
    if (p->in_vehicle) {
        ...
        float acc_speed = accel * BUGGY_MAX_SPEED; // Buggy uses different scalar logic in older code, but this works for hybrid
