...
**Status:** Milestones 1-3 shipped 2026-07-31 (Warrior's real weapon skills wired into
`arena_game.c`'s Q/W/R slots, real skillchain resonance detection between them, and a real
Battlegrounds entry point minting real IDUNA tickets for real DragonsNShit characters) —
milestones 4-5 still ahead. No GUI login path exists yet end-to-end (a player still has to run
REDGARDEN's client by hand with the printed command) — that's Milestone 4/5's territory.
