...
| 2 | Four Heavenly Kings | One real boss per camp, silent until 1:00, each granting its own distinct buff mechanic on death (§3.3: East/Music, South/Growth, West/All-Seeing, North/Wealth) | **DONE in REDGARDEN (2026-08-10)** — `ArenaKing` pool spawns silently at `ARENA_KING_SPAWN_DELAY_MS` (1:00) per camp, boss-scale HP/damage, killable by either team via `arena_hero_attack_kings()`. All 4 buffs wired into existing damage/movement/reward paths (`apply_cdr`, `update_hero_motion`, `arena_hero_bonus_ad`, `arena_hero_armor`) rather than parallel systems. West/All-Seeing's vision-reveal half honestly scoped out — no fog-of-war system exists anywhere in the sim yet (§15 is spec-only). 20 new smoke tests, `test_arena.sh` + `test_10_bots.sh` both green. REDGARDEN commit `3943f94`, Apple #12827. Not yet ported to GFD's fork (Milestone 5, below) |
...
| 5 | Validate in REDGARDEN, then port to GFD | **Reversed 2026-08-10** (see header note) — was "backport to REDGARDEN," now "REDGARDEN is the primary build target; port proven REDGARDEN implementation back to GFD's `apps2/battlegrounds_gui` fork once validated here." Milestones 1 and 2 are both done in REDGARDEN now (not yet re-synced back to GFD, which still only has Milestone 1) | Milestones 1+2 validated in REDGARDEN; port to GFD not yet started |
